...
from .models import GeneratedContent, AITask
...
class CoverLetterInputSerializer(ApplicationIdValidationMixin, serializers.Serializer):
    ...
    job_description = serializers.CharField(
        required=True,
        min_length=50,
        help_text="The full job posting text (min 50 characters)"
    )
    resume_text = serializers.CharField(
        required=False,
        help_text="Resume text. Required if resume_version_id not provided."
    )
    resume_version_id = serializers.IntegerField(
        required=False,
        help_text="ID of a saved resume version to use"
    )
    ...
    tone = serializers.ChoiceField(
        ...
        help_text="Writing tone for the cover letter"
    )
    application_id = serializers.IntegerField(
        required=False,
        help_text="Optional: Link to an existing application"
    )
    save_to_history = serializers.BooleanField(
        default=True,
        help_text="Whether to save this generation to history"
    )
    ...
    def validate(self, data):
        ...
        if not data.get('resume_text') and not data.get('resume_version_id'):
            raise serializers.ValidationError(
                "Either 'resume_text' or 'resume_version_id' must be provided."
            )
...
class JobMatchInputSerializer(ApplicationIdValidationMixin, serializers.Serializer):
    ...
    job_description = serializers.CharField(
        required=True,
        min_length=50,
        help_text="The full job posting text"
    )
    resume_text = serializers.CharField(
        required=False,
        help_text="Resume text. Required if resume_version_id not provided."
    ...
    save_to_history = serializers.BooleanField(
        default=True,
        help_text="Whether to save this analysis to history"
    )
    async_mode = serializers.BooleanField(
        required=False,
        default=None,
...
class InterviewQuestionsInputSerializer(ApplicationIdValidationMixin, serializers.Serializer):
    ...
    save_to_history = serializers.BooleanField(
        default=True,
        help_text="Whether to save to history"
    )
    async_mode = serializers.BooleanField(
        required=False,
        default=None,
