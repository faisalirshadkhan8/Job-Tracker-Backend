...
class ExportApplicationsView(APIView):
    ...
    def get(self, request):
        applications = (
            ...
            .prefetch_related("notes", "interviews")
        )
...
class ExportCompaniesView(APIView):
    ...
    def get(self, request):
        companies = Company.objects.filter(user=request.user).prefetch_related("applications")
...
class ExportInterviewsView(APIView):
    ...
    def get(self, request):
        interviews = Interview.objects.filter(application__user=request.user).select_related(
