...
class TwoFactorSetupView(APIView):
    ...
    def post(self, request):
        try:
            ...
            otpauth_url = totp.provisioning_uri(name=request.user.email, issuer_name="Job Application Tracker")
...
class TwoFactorConfirmView(APIView):
    ...
    def post(self, request):
        ...
        if TwoFactorService.confirm_setup(request.user, code):
            return Response({"message": "2FA has been enabled for your account.", "enabled": True})
        else:
...
class TwoFactorVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if TwoFactorService.verify_totp(request.user, code):
            ...
            return Response({"error": "Invalid code.", "verified": False}, status=status.HTTP_401_UNAUTHORIZED)
...
class TwoFactorDisableView(APIView):
    ...
    def post(self, request):
        ...
        if TwoFactorService.disable_2fa(request.user, code):
            return Response({"message": "2FA has been disabled for your account.", "enabled": False})
        else:
            return Response({"error": "Invalid code or 2FA not enabled."}, status=status.HTTP_400_BAD_REQUEST)
...
class BackupCodesRegenerateView(APIView):
    ...
    def post(self, request):
        ...
        if new_codes:
            return Response({"backup_codes": new_codes, "message": "Backup codes regenerated. Save these securely."})
        else:
            return Response({"error": "Invalid code or 2FA not enabled."}, status=status.HTTP_400_BAD_REQUEST)
