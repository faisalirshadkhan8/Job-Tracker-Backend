...
def retry_failed_webhooks():
    ...
    pending_retries = WebhookDelivery.objects.filter(status="retrying", next_retry_at__lte=timezone.now())
...
def cleanup_old_deliveries(days: int = 30):
    ...
    deleted, _ = WebhookDelivery.objects.filter(created_at__lt=cutoff, status__in=["success", "failed"]).delete()
