...
class CoverLetterGenerateView(APIView):
    ...
    def post(self, request):
        ...
        if data.get('application_id') and application_id is None:
            return Response(
                {'error': 'Invalid application_id for this user'},
        ...
        if not resume_text and data.get('resume_version_id'):
            ...
            if not resume_text:
                return Response(
                    {'error': 'Resume version not found or has no text content'},
                    status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def _get_resume_text(self, user, resume_version_id):
        ...
        try:
            resume = ResumeVersion.objects.get(
...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Get all your queued/completed AI tasks.",
        tags=["AI - Tasks"]
    ),
...
class AITaskViewSet(
    ...
    @extend_schema(
        ...
        description="Get all pending/processing tasks.",
        tags=["AI - Tasks"]
    )
...
class GeneratedContentViewSet(
    ...
    @extend_schema(
        ...
        description="Mark/unmark a generated content as favorite.",
        tags=["AI - History"]
    )
    @action(detail=True, methods=['post'])
    ...
    @extend_schema(
        ...
        description="Rate a generated content (1-5 stars).",
        tags=["AI - History"]
    )
    @action(detail=True, methods=['post'])
    ...
    @extend_schema(
        ...
        description="Get all favorited generated content.",
        tags=["AI - History"]
    )
    @action(detail=False, methods=['get'])
