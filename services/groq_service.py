...
class GroqService:
    ...
    def generate_cover_letter(
        cls, job_description: str, resume_text: str, company_name: str, job_title: str, tone: str = "professional"
    ) -> dict:
        """
        ...
        user_prompt = COVER_LETTER_USER_PROMPT.format(
            company_name=company_name, job_title=job_title, job_description=job_description, resume_text=resume_text
        )

        result = cls.generate_text(prompt=user_prompt, system_prompt=system_prompt, temperature=0.7, max_tokens=1500)

        return {"cover_letter": result["content"], "model": result["model"], "usage": result["usage"]}

    @classmethod
    def analyze_job_match(cls, job_description: str, resume_text: str) -> dict:
        """
        ...
        user_prompt = JOB_MATCH_USER_PROMPT.format(job_description=job_description, resume_text=resume_text)
        ...
        return {"analysis": result["content"], "model": result["model"], "usage": result["usage"]}

    @classmethod
    def generate_interview_questions(
        cls, job_description: str, company_name: str, job_title: str, question_count: int = 10
    ) -> dict:
        """
        ...
        result = cls.generate_text(
            prompt=user_prompt, system_prompt=INTERVIEW_QUESTIONS_SYSTEM_PROMPT, temperature=0.6, max_tokens=1500
        )

        return {"questions": result["content"], "model": result["model"], "usage": result["usage"]}
