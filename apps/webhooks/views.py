...
class WebhookEndpointViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(summary="Get Available Events", description="List all available webhook events.", tags=["Webhooks"])
    @action(detail=False, methods=["get"])
    def events(self, request):
        """List available webhook events."""
        return Response(
            {"events": [{"name": name, "description": desc} for name, desc in WebhookEndpoint.EVENT_CHOICES]}
        )
...
class WebhookDeliveryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = WebhookDelivery.objects.filter(endpoint__user=self.request.user).select_related("endpoint")
    ...
    @extend_schema(
        summary="Retry Webhook Delivery", description="Manually retry a failed webhook delivery.", tags=["Webhooks"]
    )
    @action(detail=True, methods=["post"])
