...
class TestPasswordReset:
    ...
    def test_password_reset_confirm(self, mock_send, api_client, db):
        ...
        data = {"token": token, "new_password": "newpass123", "new_password_confirm": "newpass123"}
        response = api_client.post(url, data, format="json")
    ...
    def test_password_reset_confirm_invalid_token(self, api_client):
        ...
        url = reverse("password_reset_confirm")
        data = {"token": "invalid-token", "new_password": "newpass123", "new_password_confirm": "newpass123"}
        response = api_client.post(url, data, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_password_reset_confirm_mismatch(self, api_client, db):
        ...
        url = reverse("password_reset_confirm")
        data = {"token": token, "new_password": "newpass123", "new_password_confirm": "differentpass"}
        response = api_client.post(url, data, format="json")
        assert response.status_code == status.HTTP_400_BAD_REQUEST
