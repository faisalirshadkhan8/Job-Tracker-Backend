...
class DashboardView(APIView):
    ...
    def get(self, request):
        ...
        active_apps = applications.exclude(
            status__in=['rejected', 'withdrawn', 'ghosted', 'accepted']
        ).count()
        ...
        apps_with_dates = applications.filter(
...
class ResponseRateView(APIView):
    ...
    def get(self, request):
        ...
        sources = Application.objects.filter(user=user).values('source').annotate(
            total=Count('id'),
            with_response=Count('id', filter=Q(response_date__isnull=False))
        ).order_by('-total')
...
class WeeklyActivityView(APIView):
    ...
    def get(self, request):
        ...
        weekly_apps = Application.objects.filter(
            user=user,
            applied_date__gte=start_date
        ).annotate(
            week=TruncWeek('applied_date')
        ).values('week').annotate(
            count=Count('id')
        ).order_by('week')
        
        return Response({
            'weekly_applications': list(weekly_apps),
            'period_weeks': weeks
        })
