...
def get_interview_data(interview):
    ...
    return {
        ...
        "company": (
            interview.application.company.name if interview.application and interview.application.company else None
        ),
...
def application_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk:
        try:
            ...
            _previous_values[f"application_{instance.pk}"] = {"status": old.status}
...
def application_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        WebhookService.dispatch_event("application.created", get_application_data(instance), instance.user_id)
    else:
        ...
        if prev.get("status") != instance.status:
            ...
            WebhookService.dispatch_event("application.updated", get_application_data(instance), instance.user_id)
...
def interview_pre_save(sender, instance, **kwargs):
    ...
    if instance.pk:
        try:
            ...
            _previous_values[f"interview_{instance.pk}"] = {"status": old.status}
...
def interview_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        WebhookService.dispatch_event("interview.created", get_interview_data(instance), user_id)
    else:
        ...
        if old_status != instance.status:
            if instance.status == "completed":
                WebhookService.dispatch_event("interview.completed", get_interview_data(instance), user_id)
            elif instance.status == "cancelled":
                WebhookService.dispatch_event("interview.cancelled", get_interview_data(instance), user_id)
            else:
                WebhookService.dispatch_event("interview.updated", get_interview_data(instance), user_id)
        else:
            WebhookService.dispatch_event("interview.updated", get_interview_data(instance), user_id)
...
def company_post_save(sender, instance, created, **kwargs):
    ...
    if created:
        WebhookService.dispatch_event("company.created", get_company_data(instance), instance.user_id)
