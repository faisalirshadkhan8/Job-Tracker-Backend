...
def analyze_job_match_task(self, task_id: int, job_description: str, resume_text: str):
    """
...
def generate_interview_questions_task(
    self, task_id: int, job_description: str, company_name: str, job_title: str, question_count: int = 10
):
    """
...
def cleanup_stale_tasks():
    ...
    stale_tasks = AITask.objects.filter(status__in=["pending", "processing"], created_at__lt=stale_threshold)

    count = stale_tasks.update(status="failed", error_message="Task timed out", completed_at=timezone.now())
